...
def linear_evaluation(root, n_classes, layer, num_epochs, split=None):
...
def get_args():
	...
	parser.add_argument("--run_id", type = str, help = "Run ID of the experiment, \
		act_label.pkl will be loaded from args.exp_type/s1-s2-s3_to_s4/args.run_id")

	parser.add_argument("--n_classes", type = int, choices = [7, 65, 31])

	parser.add_argument("--seed", type = int)

	parser.add_argument("--logs_root", help = "Location of logs folder inside JigenDG, \
		for example /DATA1/vaasudev_narayanan/repositories/JigenDG/logs")
...
if __name__ == "__main__":
	...
	logs_folder = osp.join(args.logs_root, exp_folder)
	...
	for split in splits:
		best_split_acc = linear_evaluation(logs_folder, args.n_classes, args.freeze_layer, 
			args.num_epochs, split = split)
